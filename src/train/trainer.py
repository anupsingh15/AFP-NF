...
class ModelTrainer(pl.LightningModule):
    def __init__(self,
        ...
        self.testfiles = natsorted(glob.glob(os.path.join("/home/anup/PARAMSANAGANK_backup/data/PB/test1/**/*.wav"), recursive=True))
    ...
    def step(self, batch, mode):
        ...
        prob_z_anc = self.nflows.q0.comp_log_prob(self.z_anc)
        prob_z_pos = self.nflows.q0.comp_log_prob(self.z_pos)
       
        log_prob_z_anc = torch.exp(prob_z_anc)
        log_prob_z_pos = torch.exp(prob_z_pos)
        loss_bce = -(prob_z_anc*log_prob_z_pos + prob_z_pos*log_prob_z_anc).sum(-1).sum(-1).mean() 

        loss = self.lambd[0]*loss_nf + self.lambd[1]*loss_bce
        ...
        self.log(mode+"_nf_loss", self.lambd[0]*loss_nf, on_epoch=True, on_step=True, prog_bar=True, logger=True)
        self.log(mode+"_bce_loss", self.lambd[1]*loss_bce, on_epoch=True, on_step=True, prog_bar=True, logger=True)
